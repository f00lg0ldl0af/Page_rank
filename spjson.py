...
map = dict()
ranks = dict()
for row in nodes : #(from_id COUNT, old_rank, new_rank, id, url)
    if count > 0 : fhand.write(',\n')
    # print row
    rank = row[2]
    rank = 19 * ( (rank - minrank) / (maxrank - minrank) ) # Normalise the rank to be line thickness and ball size
